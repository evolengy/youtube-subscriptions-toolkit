...
- **Периодическое обновление** — `chrome.alarms`, раз в 45 минут подтягивает новые видео по всем каналам. `apiFetch` ретраит транзиентные 5xx/429 (не 4xx) с экспоненциальным бэкоффом; цикл по каналам в `refreshAll` обёрнут в try/catch — сбой одного канала не роняет весь синк, за ним остаётся прошлый кэш. Тест `node --test youtubeApi.test.mjs`.
