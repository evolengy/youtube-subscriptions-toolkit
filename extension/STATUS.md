...
- **Управление каналами** — панель со всеми подписками, у каждой бейдж статуса активности (Active `<30д` / Quiet `<180д` / Dormant / Dead / Unknown — по последней дате в `videosCache`) + «last upload N ago», сорт по тяжести статуса; для Dead — кнопка Unsubscribe. Маленькие цветные точки статуса в списке «Assign channels». Логика — чистый модуль [channelHealth.js](channelHealth.js) (`window.YSTHealth`), `node --test channelHealth.test.js`.
