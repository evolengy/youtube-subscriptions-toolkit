...
- **Группы подписок** — создание/удаление групп, назначение каналов, эмодзи-иконка группы (поле `icon`, опциональное). Пикер — [emojiPicker.js](emojiPicker.js) (`window.YSTEmoji`) + подобранный набор ~390 эмодзи [emojiData.js](emojiData.js), поиск по имени/ключевым словам, «вставить свой» через Enter; `firstEmoji` (первая графема через `Intl.Segmenter`) покрыт `node --test emojiPicker.test.js`. Иконка видна в дашборде, в сайдбаре youtube.com и в заголовке оверлея; ставится только в дашборде ([dashboard.html](dashboard.html), [dashboard.js](dashboard.js)).
