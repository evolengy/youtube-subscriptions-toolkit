...
- **Лента по группе** с фильтрами (тип Video/Short/Live, длительность `<4 / 4–20 / >20 мин`, дата загрузки `сегодня / неделя / месяц`, текстовый поиск по заголовку+каналу), сортировкой (дата/длительность/просмотры). На карточке — icon-кнопки ✓ «Mark watched» и ⊘ «Not interested» ([icons.js](icons.js), inline SVG, `stroke: currentColor`). **«Not interested»** — extra-фильтр расширения (с YouTube не синкается), не-интересные вырезаются из ленты по умолчанию, чекбокс «Show not interested» показывает их с кнопкой ↺ Restore. `notInterestedVideoIds` в `chrome.storage.sync` (кап 2000, FIFO). Логика фильтров — общий чистый модуль [feedFilter.js](feedFilter.js) (`window.YSTFeed.applyFilters`), один на дашборд (ES-модуль) и оверлей (content-скрипт); `node --test feedFilter.test.js` (12 тестов).
